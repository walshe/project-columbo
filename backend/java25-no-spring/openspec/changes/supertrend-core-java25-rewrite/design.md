...
- Minimize third-party libraries; prefer JDK-native facilities (`java.net.http.HttpClient`, `com.sun.net.httpserver.HttpServer`, `java.sql`/JDBC, `System.Logger`, virtual threads / structured concurrency) over adding a dependency.
...
**HTTP layer**: `com.sun.net.httpserver.HttpServer` (JDK-built-in) with a small hand-written router (path + method → handler), since the endpoint count (~9) doesn't justify a framework. Query-param parsing, content negotiation (`format=JSON|MARKDOWN|WATCHLIST`), and error-body writing are explicit code, not annotations. *Alternative considered*: a micro-framework (Javalin, Helidon Nima). Rejected for now to honor "minimal libraries" as literally as possible; revisit only if the hand-rolled router becomes unwieldy in practice.

**JSON serialization**: hand-rolled, not a library. The full set of response DTOs is small (~15 flat records: primitives, `BigDecimal`, `OffsetDateTime`, enums, lists of the above) and closed — a purpose-built writer per DTO (or one small generic-enough helper for the common primitive/record cases) is tractable and keeps the "minimal libraries" goal literal. *Alternative considered*: Jackson core standalone (not Spring-coupled, so technically allowed) — rejected as a first choice since the DTO surface is small enough to hand-write; may be revisited if serialization code grows disproportionate to its value.
...
- **[Risk]** Hand-rolled JSON serialization introduces subtle formatting bugs (number precision, date-time format, null handling) that a mature library wouldn't. → **Mitigation**: contract/characterization tests comparing new-impl JSON output byte-for-byte (modulo key ordering) against the old app's actual responses for the same seeded data, for every endpoint in scope.
- **[Risk]** Hand-rolled HTTP routing/dispatch could accumulate ad-hoc special cases as endpoints are added (content negotiation, error mapping) and end up less consistent than a framework's conventions. → **Mitigation**: extract a small shared `RequestContext`/`ResponseWriter` pair early so every handler follows the same shape; revisit "minimal libraries" if this grows past ~2-3 handlers' worth of duplicated logic.
...
- **[Trade-off]** Retaining Flyway (a real dependency) slightly contradicts "minimal libraries taken literally," but hand-rolling migration tracking for a genuinely evolving schema is judged lower-value than the JSON/HTTP hand-rolling above, where the DTO/endpoint surface is small and closed.
...
3. Validate each read API's output against the equivalent `backend/java` endpoint for the same underlying data (characterization testing), per the JSON-serialization risk mitigation above.
4. No rollback concerns beyond normal within-change git revert, since nothing external depends on this implementation yet.

## Open Questions

- **JSON serialization approach**: hand-rolled per-DTO writer vs. one small generic reflection-free helper shared across records — decide during implementation of the first API capability, once the actual DTO shapes are drafted; not blocking for tasks.md.
- **Exact package names**: sketched above (`indicator`, `ingestion`, `signal`, `pulse`, `pipeline`, `api`, `persistence`, `freshness`) as a starting point, not a final layout — expected to be refined once real classes exist and SOLID seams become concrete.
- **Flyway vs. hand-rolled migrations**: leaning Flyway (see Decisions) but not firmly locked in; revisit if the schema stays small enough that hand-rolling is trivial.
- **Whether to seed test/dev data from `backend/java`'s existing asset list** (the ~60 assets from its `V11` migration) as a convenience, vs. defining an independent seed list from scratch — doesn't block implementation order, decide when ingestion capability is built.
