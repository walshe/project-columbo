...
- Actuator-style health/metrics endpoints, Micrometer.
...
**HTTP layer** *(revised during group 9 implementation)*: [Javalin](https://javalin.io) (`io.javalin:javalin`), not the originally-planned hand-rolled `com.sun.net.httpserver.HttpServer` router. Reason for the change: the owner wants a client hooked up to this API in the future and wants a proper OpenAPI/Swagger-compliant surface for that — which was explicitly out of scope in the original Non-Goals ("OpenAPI doc generation/UI"), now removed from that list. Javalin was chosen over Quarkus specifically to preserve this rewrite's other stated principles: no DI container, no build-time bytecode augmentation, and an explicit hand-wired composition root identical to the rest of this codebase — Javalin is a thin routing layer (Jetty underneath), not an application framework. OpenAPI spec + Swagger UI are provided by the `io.javalin.community.openapi` plugin family (`javalin-openapi-plugin`, `javalin-swagger-plugin`, plus the `openapi-annotation-processor` compile-time annotation processor — no reflection at runtime). Query-param validation uses Javalin's built-in `Context.queryParamAsClass(...)`/`Validator` machinery rather than a hand-written parser; a case-insensitive converter is registered for `Timeframe`. Error-response mapping (400/404/409/503) uses Javalin's built-in `HttpResponseException` subclasses (`BadRequestResponse`, `NotFoundResponse`, etc. — auto-mapped, no registration needed) plus explicit `app.exception(...)` handlers for this codebase's own domain exceptions (`IngestionAlreadyRunningException` → 409, `StaleDataException` → 503 with `Retry-After`), replacing the originally-planned fully-hand-rolled router/dispatcher.
