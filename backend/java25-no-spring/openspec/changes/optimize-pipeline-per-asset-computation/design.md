...
Step 3 is already incremental. Steps 1–2 are not. Run cost is therefore O(total history × active assets) every run, regardless of how little changed. `SuperTrendCalculator.calculateIncremental` already establishes the basis for doing better: it recomputes new rows over only a `WARMUP_WINDOW_BARS` (`atrLength * 10`) slice before the anchor, which is enough for the trend **direction** to restabilize identically to a full recompute (the Wilder-ATR magnitude reconverges to within ~1e-4 — a decaying EMA residue — which never moves a direction). Today that window is applied only to *slice the calculator's input list* — the DB load ahead of it is still the whole history, and the signal pass doesn't use `calculateIncremental` at all.
...
- **Indicator path:** bit-for-bit identical stored `indicator_supertrend` rows vs. today. Free — `calculateIncremental` *already* recomputes new rows over only a `WARMUP_WINDOW_BARS` slice it cuts internally; bounding the DB load just stops fetching rows it was going to discard.
- **Signal path:** identical `signal_state` rows (trend state + flip event) vs. today's full recompute, for every candle at/after each asset's anchor, on an append-only history. `signal_state` persists only the direction-derived state; direction is unaffected by the sub-1e-4 Wilder-ATR/band residue a shorter warm-up leaves (see Risks), and the ATR/band magnitudes are not persisted here.
...
- **[Trade-off] The signal path moves from a true full recompute to a `WARMUP_WINDOW_BARS` window, so its in-memory ATR/bands carry a sub-1e-4 Wilder-EMA residue vs. a from-scratch recompute.** → `signal_state` stores only trend state (`BULLISH`/`BEARISH`/`UNKNOWN`) and flip event, both derived from `direction`, which is a sign test comfortably clear of that residue - verified byte-identical over a 300-bar multi-flip series in `SuperTrendCalculatorTest`. Bonus: signal-state directions are now computed from the *same* windowed input as the `indicator_supertrend` rows (which `calculateIncremental` already windows), so the two can no longer drift apart the way a full-recompute signal pass could in principle.
...
- **[Risk] `warmupBars` too small → bounded trend direction diverges from a full recompute.** → `atrLength * 10` is the constant `calculateIncremental` has used in production since the rewrite; this change doesn't change it, only applies it to the read (indicator path) and the signal pass. Mitigation: `SuperTrendCalculatorTest` computes a 300-bar multi-flip series both ways and asserts (a) `calculateIncremental` over a pre-bounded window is byte-identical to over full history — the indicator-path guarantee — and (b) `calculate` over the bounded window yields identical `direction` for every candle at/after a mid-series anchor — the signal-path guarantee.
