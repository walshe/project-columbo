...
- [x] 3.2 Run `mvn verify -Pe2e` locally: inconclusive due to environment load. `PipelineEndToEndIT` failed/hung with `HikariPool - Connection is not available ... (total=10, active=10, idle=0, waiting=194+)` cascading across both Binance-tokenized assets (`AAOIUSDT`, `AMDUSDT`, `GMEUSDT`, `DAIUSDT`, `SKHYNIXUSDT`, ...) and a Tiingo asset (`MA`) - a real regression in the Tiingo-only volume change could not affect Binance assets at all, so this is host resource contention (confirmed separately: ~66-427MB free host RAM, ~15 unrelated Docker containers already running) exhausting the pool under the pipeline's ~247-virtual-thread fan-out against a 10-connection pool, not a code defect. Confirmed on PR #70's GitHub Actions run (clean runner, no contention): `e2e` passed in ~4-5 minutes (normal runtime) on both duplicate CI runs.
- [x] 3.3 Self-review before opening the PR. `pr_agent` on PR #70 raised that the backfill assumes every stored `TIINGO`-provider candle's `close` is already the adjusted close, and flagged this as an unenforced assumption. Checked: `TiingoMarketDataProvider.toCandle` has used `adjClose` unconditionally since `add-tiingo-provider` (the only ingestion path for Tiingo data - no alternate/manual insert path exists anywhere in this codebase), so there is no actual scenario where a stored Tiingo candle's `close` isn't already adjusted. No code change needed.
