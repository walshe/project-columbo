...
/**
 * Processes a single asset's SignalState detection in its own transaction.
 * Split out from {@link SignalStateService} so that REQUIRES_NEW actually takes
 * effect — calling this as {@code this.processAsset(...)} from within
 * SignalStateService would bypass the Spring proxy (self-invocation) and silently
 * run everything in one long-lived transaction/connection instead.
 */
@Service
public class SignalStateAssetProcessor {
    private static final Logger log = LoggerFactory.getLogger(SignalStateAssetProcessor.class);
    ...
    public SignalStateAssetProcessor(CandleRepository candleRepository,
